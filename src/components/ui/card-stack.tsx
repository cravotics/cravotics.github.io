            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    'absolute bottom-0 rounded-2xl border border-border overflow-hidden shadow-xl',
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on ? 'w-6 bg-accent' : 'w-2 bg-dim hover:bg-accent/60'
                  )}
    ...
    <div className="relative h-full w-full bg-surface-2">
      <div className="absolute inset-0">
        {item.imageSrc ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-sm text-dim font-mono">
            No image
          </div>
        )}
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-5">
        {item.tag ? (
          <span className="mono-tag self-start mb-2 text-[10px]">{item.tag}</span>
        ) : null}
        <div className="truncate text-lg font-bold font-mono text-white">{item.title}</div>
        {item.description ? (
